...
function Configure-FudgePack {
	[CmdletBinding()]
	param ()
	try {
		$st = Get-ScheduledTask -TaskName "Run FudgePack" -ErrorAction SilentlyContinue
		if ($st) {
			$tr = $st.Triggers
	...
	try {
		$logsize = Get-ItemProperty -Path "HKLM:\SOFTWARE\FudgePack" -Name "MaxLogSizeMB" -ErrorAction SilentlyContinue | Select -ExpandProperty MaxLogSizeMB
		$xx = (Read-Host -Prompt 'Maximum log file size in MB [$logsize]')
		if ($xx -eq $null -or $xx -eq '') { $xx = $logsize }
		Set-ItemProperty -Path "HKLM:\SOFTWARE\FudgePack" -Name "MaxLogSizeMB" -Value $xx
	}
	catch {
